...
@Mock(Airplane)
class AirplaneControllerSpec extends Specification {
    ...
    void "Test that the show action returns the correct model"() {
        ...
        when:"A domain instance is passed to the show action"
            ...
            controller.show(airplane)

        then:"A model is populated containing the domain instance"
            model.airplane == airplane
    }
    ...
    void "Test the update action performs an update on a valid domain instance"() {
        ...
        when:"A valid domain instance is passed to the update action"
            response.reset()
            populateValidParams(params)
            airplane = new Airplane(params).save(flush: true)
            controller.update(airplane)

        then:"A redirect is issued to the show action"
            airplane != null
            response.redirectedUrl == "/airplane/show/$airplane.id"
            flash.message != null
    }
    ...
    void "Test that the delete action deletes an instance if it exists"() {
        ...
        when:"A domain instance is created"
            response.reset()
            populateValidParams(params)
            def airplane = new Airplane(params).save(flush: true)

        then:"It exists"
            Airplane.count() == 1

        when:"The domain instance is passed to the delete action"
            controller.delete(airplane)
